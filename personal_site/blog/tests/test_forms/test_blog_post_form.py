...
class TestBlogPostForm(TestCase):
    ...
    def test_blog_post_form_valid(self):
        form = BlogPostForm(data={'title': 'test', 'content': 'test',
                                  'category': self.category.pk, 'is_published': True}, user=self.user)
    ...
    def test_blog_post_form_save(self):
        form = BlogPostForm(data={'title': 'test_save', 'content': 'test',
                                  'category': self.category.pk, 'is_published': True}, user=self.user)
        form.is_valid()
        post = form.save()
        self.assertTrue(BlogPost.objects.filter(title='test_save').exists())
        self.assertTrue(HistoryLog.objects.filter(source=f'NEW:{post}', action='Blog created').exists())
    
    def test_blog_post_form_update(self):
        form = BlogPostForm(
            instance=self.blog_post, data={
                'title': 'test_update', 'content': 'test', 
                'category': self.category.pk, 'is_published': True}, user=self.user)
        form.is_valid()
        form.save()
        self.assertTrue(BlogPost.objects.filter(title='test_update').exists())
        self.assertTrue(HistoryLog.objects.filter(
            source=f'{self.blog_post.pk}:{self.blog_post}', action='Blog updated').exists())
